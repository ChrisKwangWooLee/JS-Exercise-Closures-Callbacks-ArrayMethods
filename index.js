...
function processDuplicateFree(list, callback) {
  let uniqueList = list.filter((el, i) => {
...
function getFullNames(runners) {
  let fullName = [];
  runners.forEach(el => {
    fullName.push(`${el.last_name}, ${el.first_name}`);
  })
  return fullName;
}
...
function tallyUpDonations(runners) {
  let sumDon = runners.reduce((accumulator, el) => {
    return accumulator + el.donation;
  }, 0);
  return sumDon;
}
